...
def test_local_timezone_resolver_accepts_injected_named_sources(
    ...
    import workfold.time_ranges as time_ranges

    missing = tmp_path / "missing"
    assert resolve_local_timezone(
        environ={"TZ": "Europe/Berlin"}, timezone_file=missing, localtime_file=missing
    ).key == ("Europe/Berlin")
    monkeypatch.setattr(time_ranges, "_tzlocal_zone_name", lambda: None)
...
def test_local_timezone_resolver_can_use_zoneinfo_from_current_datetime(
    ...
    monkeypatch.setattr(time_ranges, "_tzlocal_zone_name", lambda: None)
    monkeypatch.setattr(time_ranges, "datetime", _ZoneInfoNow)
    missing = tmp_path / "missing"
...
def test_tzlocal_adapter_failure_is_accounted(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(time_ranges, "get_localzone_name", fail_resolution)
    adapter = getattr(time_ranges, "_tzlocal_zone_name")
...
def test_local_timezone_resolver_rejects_fixed_offset_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(time_ranges, "_tzlocal_zone_name", lambda: None)
    monkeypatch.setattr(time_ranges, "datetime", _FixedOffsetNow)
    missing = tmp_path / "missing"
