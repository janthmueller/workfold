"""Canonical mechanical schema for every configurable Workfold setting."""
...
if len(SETTING_BY_KEY) != len(SETTING_SPECS):
    raise RuntimeError("Workfold setting keys must be unique")
if len(SETTING_BY_DESTINATION) != len(SETTING_SPECS):
    raise RuntimeError("Workfold setting CLI destinations must be unique")
