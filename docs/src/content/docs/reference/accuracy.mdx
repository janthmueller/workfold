...
timestamp kinds, commit reachability, authors, filesystem ignore policy,
explicit exclusions, and platform capabilities. `--coverage` prints the
detailed ledger without operational details; `--verbose` prints operational
...
coverage exception for active author filtering, explicit exclusions,
...
the underlying markers, source/schedule totals, and coverage ledger are unchanged.
...
Reports can still contain local paths, author identities, commit subjects,
...
For the exact accounting contract, see the
[MVP specification](https://github.com/janthmueller/workfold/blob/main/MVP.md).
