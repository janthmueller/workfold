...
selector must cover both retained roles to apply; Workfold never discards a role
...
- Deleted untracked entries and earlier mutable metadata values are
  ...
  reconstructed without a watcher, which Workfold does not provide.
...
- Git is invoked without fetching or contacting remote hosting APIs; Workfold
...
- Workfold has no telemetry, persistent activity database, background watcher,
  or web server;
