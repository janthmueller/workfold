...
That scope includes paths, the time selector, mode, evidence preset, record and
...
and available nanoseconds remain in provenance and exact outside-event
listings.
...
- Git file changes inherit their commit's selected timestamp roles. Workfold
  therefore derives tree differences only for commits with at least one
  matching role; this is an exact scope optimization, not a history shortcut.
