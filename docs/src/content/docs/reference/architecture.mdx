...
Workfold keeps four independent questions separate instead of encoding all
scope in one flag:
...
`CollectionPlan` projects it into exact commit, file-change, tag, reflog, and
filesystem entry-type/timestamp work; collectors do not reinterpret CLI
presets or keep a second scope model. Filesystem record discovery remains one
