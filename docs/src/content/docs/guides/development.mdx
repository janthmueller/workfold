...
observation or an explicit materialization error.

Git revision output is streamed. A lightweight exact scan applies the requested
author/committer roles independently, and commit objects plus tree differences
are hydrated in bounded batches only for matching commits. This remains exact
for file-change observations because they inherit those commit roles. Do not
replace author-date selection with ordinary `git --since`: author dates are not
topologically monotonic. Filesystem discovery still inventories and stats every
candidate required by entry and ignore scope, but it need not materialize a
normalized observation for a known out-of-range timestamp. Production runs do
not retain the full record or observation inventory after its counters have
been updated. Batch sizes are memory controls, never collection, query, or chart
semantics.
