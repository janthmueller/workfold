...
The public `collectors.git`, `collectors.git_objects`,
`collectors.git_changes`, and `git_core.commits` modules are thin import
façades for the current collector API. Alpha releases do not retain obsolete
parser signatures solely through these façades. Implementation modules in
`collectors/git_core/` separate immutable models and accounting, lightweight
revision scans, raw commit parsing, `cat-file` envelope handling, bounded
object extraction, per-repository commit hydration, provisional change
spooling, subprocess-stream lifecycle, and multi-repository collector
orchestration. Collector internals import those focused modules directly. Keep
protocol state machines independent from domain conversion and coverage; do
not grow the façades back into multi-purpose implementation modules.
...
`config_types` owns immutable option values and built-in defaults,
`config_parsing` owns individual value grammars, and the public `config` module
owns cross-option resolution and re-exports the supported configuration API.
Likewise, `settings.cli_parser` declares argparse presentation while `cli`
handles configuration layering and runtime dispatch. Keep argparse details out
of domain validation.

TOML defaults are resolved before `RawOptions` validation as built-ins, the
...
Coverage data and coverage prose are also separate. `coverage_models` defines
immutable partitions and reconciliation invariants; `coverage_builder` owns
mutable collection-time accumulation; `coverage` is their public façade. In
the application layer, `coverage_scope` names requested evidence,
`coverage_status` produces the compact completeness statement, and
`coverage_text` composes the verbose ledger.
...
must still pass a current no-follow `lstat`, followed by the same timestamp
adapter used by native traversal. An index-only path absent from the current
worktree is therefore not filesystem evidence. Directory scans combine the
same inventory's ignored-directory boundaries with the native no-follow walker,
...
`FilesystemCollector` resolves and schedules roots; `filesystem.root_collector`
...
Pathologically busy cells retain exact
counts per visual kind instead of an unbounded alternating sequence. Chart
sorting stays in memory for normal runs and transparently spills to an
ephemeral local disk-backed sorter above its fixed threshold; the temporary
data is removed when aggregation finishes. Only the renderer creates Rich
