...
Configuration resolves time independently from evidence scope. Common evidence
scope uses the mode/profile preset pair; exact `--events` selectors are a
...
`CollectionPlan` rather than retaining parallel record/timestamp flags. The
preset defaults to `standard` and never rewrites time or mode; `portable` is
intentionally valid only with Git mode.
...
independent members of the requested root union.
