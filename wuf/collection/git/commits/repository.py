...
from wuf.collection.git.objects.reader import (
    GitObjectReadError,
    iter_spooled_commit_objects,
    spool_commit_candidate,
)
from wuf.collection.git.objects.revision_scan import inspect_rev_list_scan
from wuf.collection.git.repository import GitRepository
from wuf.collection.git.revisions import iter_commit_scans_for_contexts
                        ...
                        if isinstance(result, UnavailableBatchObject):
                            ...
                            diagnostics.append(
                                CollectorDiagnostic(
                                    ...
                                    hint=(
                                        "The repository may be shallow or partial; Wuf will not fetch missing objects."
                                    ),
                                )
                            )
                            continue
