...
from wuf.collection.diagnostics import CollectorDiagnostic, DiagnosticCategory
from wuf.collection.git.command_error import GitCommandError
from wuf.collection.git.process import open_git_stdout
    ...
    def _command(self, arguments: Sequence[str], *, cwd: Path) -> tuple[str, ...]:
        if not arguments or arguments[0] not in LOCAL_READ_COMMANDS:
            ...
            raise GitCommandError(
                ...
                cwd=cwd,
                hint="Wuf only invokes allow-listed, read-only Git commands.",
            )
