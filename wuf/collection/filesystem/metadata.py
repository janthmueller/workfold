...
from wuf.domain.coverage import Capability, CapabilityKind, CapabilityStatus, ExtractionDisposition
from wuf.domain.observations import Source, TimestampKind
    ...
    *,
    ...
    if not _MIN_LOCALIZABLE_NS <= value < _MAX_LOCALIZABLE_NS:
        return TimestampExtraction(
            ...
            ExtractionDisposition.ERROR,
            field_name=field_name,
            note="timestamp is outside Wuf's safely localizable datetime range",
        )
    return TimestampExtraction(
        kind,
