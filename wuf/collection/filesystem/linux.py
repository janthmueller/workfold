...
This module binds only the libc ``statx`` surface Wuf needs and keeps the
...
class _Statx(ctypes.Structure):
    ...
    # offsets of every field Wuf reads.
    _layout_ = "ms"
    _pack_ = 1
    _fields_ = [
...
if ctypes.sizeof(_Statx) != _STATX_BUFFER_SIZE:  # pragma: no cover - fixed Linux UAPI invariant
    raise RuntimeError("Wuf's Linux statx structure does not match the 256-byte UAPI layout")
...
class LinuxStatxReader:
    ...
    def read_snapshot(self, path: Path) -> LinuxStatxSnapshot:
        """Read all metadata Wuf needs without a companion ``lstat`` call."""
